...
function DirectMessageBubble({
  ...
  useEffect(() => {
    if (!currentMessage.seenBy?.[user.uid] && inView) {
      currentMessage.updateLastVisitedBy();
    }
  }, [currentMessage, inView, user.uid]);
  ...
  const replySender = useMemo(() => {
    ...
    return {
      id,
      name,
      avatar: selectedDirectMessageChat!.members[id]?.avatar,
    };
  }, [currentMessage, selectedDirectMessageChat]);

  const avatar = useMemo(
    () => selectedDirectMessageChat!.members[currentMessage.sender.id]?.avatar,
    [currentMessage, selectedDirectMessageChat]
  );
